...
(function () {
    ...
    mutations.forEach(function (mutation) {
      ...
      if (type === "attributes") {
        ...
        if (
          ...
          target.getAttribute(mutation.attributeName) === null
        ) {
        ...
        dispatchMutationEvent("DOMCharacterDataModified", target, {
        ...
        mutation.removedNodes.forEach((n) => {
          ...
          walk(n, (node) =>
            dispatchMutationEvent("DOMNodeRemovedFromDocument", node, {
              bubbles: false,
            })
          );
        ...
        mutation.addedNodes.forEach((n) => {
          ...
          walk(n, (node) =>
            dispatchMutationEvent("DOMNodeInsertedIntoDocument", node, {
              bubbles: false,
            })
          );
        });
  ...
  function enableMutationEventPolyfill(target) {
    if (hasTarget(listeningNodes, target)) return;
  ...
  function disableMutationEventPolyfill(target) {
    ...
    const rootElement = getRootElement(target);
    if (!hasTarget(documentsToObservers, rootElement)) return;
  ...
  Element.prototype.addEventListener = function (eventName, listener, options) {
    if (hasTarget(mutationEvents, eventName)) {
      ...
      const { augmentedListener, fullEventName } = getAugmentedListener(
        eventName,
        listener,
        options
      );
  ...
  Element.prototype.removeEventListener = function (
    ...
    if (hasTarget(mutationEvents, eventName)) {
      ...
      originalRemoveEventListener.apply(this, [
        fullEventName,
        augmentedListener,
        options,
      ]);
  ...
  console.log(
    `Mutation Events polyfill installed (native feature: ${
      nativeFeatureSupported ? "supported" : "not present"
    }).`
  );
})();
