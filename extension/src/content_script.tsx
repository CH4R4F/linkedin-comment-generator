const commentSection = document.querySelectorAll(
...
function insertGenerationButton(commentSection: HTMLElement) {
  console.log(commentSection);
  ...
  const closestComment = commentSection.closest(
    ".comments-comment-entity--reply"
  ); /* check if there is a parent with class "comments-comment-entity--reply" */

  if (closestComment) {
    data.isReply = true;
    data.repliedCommentContent = closestComment?.querySelector(
      ".comments-comment-item-content-body"
    )?.textContent!;
  }

  // const postParent = commentSection.closest(".feed-shared-update-v2");
  ...
  // commentSection.insertAdjacentElement("afterbegin", wrapper);
  // wrapper
  //   ?.querySelector("button")
  //   ?.addEventListener(
  //     "click",
  //     async () => await generateComment(data, commentSection)
  //   );
}
...
async function generateComment(data: any, commentSection: HTMLElement) {
  ...
  const commentContainer =
    commentSection.previousElementSibling?.querySelector(".ql-editor");

  // // send this data to background script
  // const response = await chrome.runtime.sendMessage({
  //   message: "generateComment",
  //   data,
  // });

  // comment.textContent = response?.trim();
  // commentContainer?.innerHTML && (commentContainer.innerHTML = "");
...
document
  .querySelectorAll(".comments-comment-texteditor")
  .forEach(insertGenerationButton as any);
